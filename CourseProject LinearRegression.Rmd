...
## Материалы и методы
Исходные данные - наблюдения толщины снежного покрова на нескольких станциях Owens Valley и объемов речного стока вблизи Bishop, California.
Структура исходных данных:

    Year - тип данных num - год сбора данных;
    APMAM - тип данных num - снежный покров в дюймах на станции APMAM;
    APSAB - тип данных num - снежный покров в дюймах на станции APSAB;
    APSLAKE - тип данных num - снежный покров в дюймах на станции APSLAKE;
    OPBRC - тип данных num - снежный покров в дюймах на станции OPBRC;
    OPRC - тип данных num - снежный покров в дюймах на станции OPRC;
    OPSLAKE - тип данных num - снежный покров в дюймах на станции OPSLAKE;
    BSAAM - тип данных num - объем речного стока около Bishop, California, в акро-футах.
Всего 43 наблюдения. Пропусков в данных нет.

Структура анализа:

- разведочный анализ данных (характер связи между откликом и предикторами, распределение зависимой переменной, влиятельные наблюдения, независисмость предикторов);
- моделирование;
- проверка предикторов на мультиколленеарность;
- проверка валидности модели;
- упрощение модели.

Расчеты проведены в среде R [4, 5]. Для построения графиков использовались пакеты ggplot2 [6] и cowplot[7]. Для импорта данных использовался пакет readxl [8]. Пакет tidyr [9] использовался для пробразования данных. Отчет подготовлен с помощью пакета knitr [10].
...
До начала регрессионного анализа необходимо провести разведочный анализ для определения размаха данных и характера взаимосвязи между зависимой переменной - объема речного стока BSAAM, и независимыми - грубине снежного покрова на 6 станциях: APMAM, APSAM, APSLAKE, OPBPC, OPRC, OPSLAKE.

```{r echo=FALSE, fig.height=3, fig.width=10}
    plot_a
```

__Рис.1.__ Диаграммы разведочного анализа исходных данных


По диаграммам разведочного анализа (Рис.1) видно, что связь между зависимой и независимым переменными линейная, выбросы данных есть, но в рамках каждой станции их немного. Это позволяет
 выдвинуть гипотезу, что взаимосвязь между глубиной снежного покрова и объемом речного стока, может быть описана с помощью модели множественной регрессии.
